...
const Todoitem = ({task, del, set}) => {
  ...
  return (
    ...
    <div className='todoitem my-3'>
      ...
      {/* </div> */}
       <button type="button" className={`btn${task.clicked?' btn-secondary':' btn-outline-secondary'}`} id={`${task.clicked?'tbtn':'btnn'}`} onClick={()=>{set(task.id)}}>{task.clicked?'Reset':'Done'}</button>
      </div>
